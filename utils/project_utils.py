...
class My_model():
    ...
    def __init__(self,loaded_model = False,saved_model_dir=None):
    ...
    def fit_model(self,x_train,y_train,x_val,y_val,epochs=10):
        """Train model
        Params:
            x_train: Training x
            y_train: Training y
...
def get_adversarial_data(set_type = "Test_fgsm"):
    """
...
def compile_model(model):
    try:
        model.compile(loss= 'categorical_crossentropy', optimizer = 'rmsprop', metrics = ['accuracy',tf.keras.metrics.Precision(), tf.keras.metrics.Recall()])
        print("Model compiled")
    except:
        print("Error with your model or compilation")
